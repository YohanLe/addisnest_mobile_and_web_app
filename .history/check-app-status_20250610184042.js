...
async function checkApplicationStatus() {
  ...
  try {
    const data = await httpGet('http://localhost:7000/');
    ...
    try {
      ...
      const data = await httpGet('http://localhost:7000/api/users');
      log(`✅ API endpoint '/api/users' is accessible`, 'green');
    } catch (secondError) {
      try {
        // Try one more endpoint
        const data = await httpGet('http://localhost:7000/api/properties');
        log(`✅ API endpoint '/api/properties' is accessible`, 'green');
      } catch (thirdError) {
        log(`❌ API endpoints are not accessible`, 'red');
      }
    }
  }
