...
const ChoosePromotion = () => {
  ...
  const savePropertyToDatabase = async (data, plan) => {
    try {
      ...
      // Format property data for API call - FIXED VERSION
      ...
      const formattedData = {
        ...
        // NOTE: Don't include status or paymentStatus - the server will set them
        // and was causing 500 error by overriding with invalid values
        ...
        isPremium: false,
        isVerified: false,
        promotionType: plan === 'basic' ? 'Basic' : plan.charAt(0).toUpperCase() + plan.slice(1),
        views: 0,
        likes: 0,
  ...
  const handleContinue = async () => {
    ...
    try {
      ...
      if (isTestMode) {
        ...
        try {
          // FIXED: Transform form data to match the backend schema format
          const transformedData = {
            title: dataToUse.title || "",
            description: dataToUse.description || "",
            propertyType: dataToUse.propertyType || "Apartment",
            offeringType: dataToUse.offeringType || "For Sale",
            price: Number(dataToUse.price) || Number(dataToUse.total_price) || 0,
            area: Number(dataToUse.area) || Number(dataToUse.property_size) || 0,
            bedrooms: Number(dataToUse.bedrooms) || Number(dataToUse.number_of_bedrooms) || 0,
            bathrooms: Number(dataToUse.bathrooms) || Number(dataToUse.number_of_bathrooms) || 0,
            furnishingStatus: dataToUse.furnishingStatus || dataToUse.furnishing || "Unfurnished",
            address: {
              street: dataToUse.address?.street || dataToUse.property_address || "",
              city: dataToUse.address?.city || dataToUse.city || "",
              state: dataToUse.address?.state || dataToUse.regional_state || "",
              country: dataToUse.address?.country || dataToUse.country || "Ethiopia"
            },
            // Transform media_paths to images format
            images: dataToUse.images || 
              (dataToUse.media_paths ? 
                dataToUse.media_paths.map(url => typeof url === 'string' ? {url, caption: ''} : url) : 
                [])
          };
          
          savedProperty = await savePropertyToDatabase(transformedData, selectedPlan);
