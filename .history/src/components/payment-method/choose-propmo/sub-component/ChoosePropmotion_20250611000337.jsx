...
const ChoosePromotion = () => {
  ...
  const savePropertyToDatabase = async (data, plan) => {
    try {
      ...
      const formattedData = {
        ...
        // For all plans, set initial status as "pending"
        // For Basic plan, paymentStatus will be updated to "active" immediately after submission
        // For VIP/Diamond plans, paymentStatus remains "pending" until payment completion
        status: 'pending',
  ...
  // No required field validation here; assume property form enforces required fields before navigation.
    ...
    try {
      ...
      // For Basic Plan (free), update payment status to active and navigate to confirmation page
      if (selectedPlan === 'basic') {
        try {
          // Update the payment status to active for Basic plan
          console.log("Updating payment status to active for Basic plan property:", savedProperty._id);
          await Api.patchWithtoken(`properties/${savedProperty._id}`, {
            paymentStatus: 'active',
            status: 'active'
          });
          console.log("Payment status updated to active for Basic plan");
...
{`// Payment Flow:
// 1. All properties initially saved with status="pending", paymentStatus="pending"
// 2. For Basic Plan: Update to status="active", paymentStatus="active" immediately
// 3. For VIP/Diamond Plans: Status remains pending until payment completed
// 4. Upon successful payment, update status and paymentStatus to "active"`}
          </pre>
        </div>
      </div>
    </div>
