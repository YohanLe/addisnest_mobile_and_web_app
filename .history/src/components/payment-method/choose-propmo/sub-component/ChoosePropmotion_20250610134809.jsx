...
const ChoosePromotion = () => {
  ...
  const savePropertyToDatabase = async (data, plan) => {
    try {
      ...
      // Create the formatted data structure expected by the API (flat structure)
      const formattedData = {
        title: data.title || data.property_address || '',
        description: data.description || '',
        propertyType: data.property_type || '',
        offeringType: data.property_for === "For Sale" ? "For Sale" : "For Rent",
        status: 'active',
        paymentStatus: 'none',
        price: Number(data.total_price) || 0,
        area: data.property_size ? Number(data.property_size) : 0,
        ...
        images: (data.media_paths || []).map((path, idx) => ({
          url: typeof path === 'string' ? path : path.filePath || '',
          caption: `Image ${idx + 1}`
        })),
        isPremium: false,
        isVerified: false,
        promotionType: plan === 'basic' ? 'None' : plan.charAt(0).toUpperCase() + plan.slice(1),
        views: 0,
        likes: 0
      };
