...
const ChoosePromotion = () => {
  ...
  const savePropertyToDatabase = async (data, plan) => {
    try {
      ...
      const propertyForValue = data.property_for === "For Sale" ? "sale" : "rent";
      
      // Create the formatted data structure expected by the API
      const formattedData = {
        // Basic property information
        title: `${data.property_type} for ${propertyForValue} in ${data.city || 'Unknown Location'}`,
        description: data.description || `${data.property_type} property available for ${propertyForValue}.`,
        propertyType: data.property_type?.toLowerCase() || 'house',
        propertyFor: propertyForValue,
        offering: `for-${propertyForValue}`,
        status: 'active',
        ...
        price: {
          ...
          currency: 'ETB',
          period: propertyForValue === 'rent' ? 'monthly' : 'one-time'
        },
        ...
        location: {
          ...
          city: data.city || '',
          state: data.regional_state || '',
          country: data.country || 'Ethiopia'
        },
        
        // Property specifications
        specifications: {
          bedrooms: data.number_of_bedrooms ? Number(data.number_of_bedrooms) : 0,
          bathrooms: data.number_of_bathrooms ? Number(data.number_of_bathrooms) : 0,
          area: {
            size: data.property_size ? Number(data.property_size) : 0,
            unit: 'sqm'
          }
        },
      ...
      // Add furnishing status if provided
      if (data.furnishing) {
        formattedData.features.furnishingStatus = data.furnishing.toLowerCase();
      }
      
      // Add plan details
      formattedData.isFeatured = plan !== 'basic';
      formattedData.priority = plan === 'diamond' ? 10 : (plan === 'vip' ? 5 : 0);
      
      // Add image paths
      if (data.media_paths && Array.isArray(data.media_paths)) {
        formattedData.images = data.media_paths.map((path, index) => {
          return {
            url: typeof path === 'string' ? path : path.filePath || '',
  ...
  // Check if all required fields exist in the property data - but only log, don't show errors
  if (!isTestMode) {
    const missingFields = requiredFields.filter(field => 
      !propertyData[field] || 
      (field === 'media_paths' && (!Array.isArray(propertyData[field]) || propertyData[field].length === 0))
    );
    
    if (missingFields.length > 0) {
      console.log("Missing required fields:", missingFields);
      // The form will handle validation so we don't need to display errors here
    } else {
      console.log("All required fields present for property submission");
    }
  }
    ...
    try {
      ...
      // Instead of saving to the database, we'll just use the local data
      // and simulate a successful save operation
      console.log("Skipping actual database save - using local data only");
      
      // Use the same data format that would be returned by the API
      const savedProperty = isTestMode 
        ? simulateSavePropertyInTestMode(dataToUse, selectedPlan)
        : simulateSavePropertyInTestMode(dataToUse, selectedPlan); // Use simulation for both cases
      
      // Add a log to show we're using simulated data
      console.log("Using simulated property data:", savedProperty);
