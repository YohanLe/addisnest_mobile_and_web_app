  ...
  const handleCompletePayment = async () => {
    ...
    setTimeout(async () => {
      ...
      if (propertyData && propertyData._id && (plan === 'vip' || plan === 'diamond')) {
        try {
          // Update property status and paymentStatus to "active"
          // You may need to adjust the API endpoint/method as per your backend
          await import('../../../../Apis/Api').then(({ default: Api }) =>
            Api.putWithtoken
              ? Api.putWithtoken(`properties/${propertyData._id}`, { status: 'active', paymentStatus: 'active' })
              : Api.patchWithtoken(`properties/${propertyData._id}`, { status: 'active', paymentStatus: 'active' })
          );
        } catch (err) {
          updateSuccess = false;
