...
const PropertyListingsTab = () => {
  ...
  // Get property list data from Redux store and local storage
  ...
  // Prioritize localStorage data, only use API data if localStorage is empty
  const combinedPropertyList = localStorageProperties.length > 0 
  ...
  // Fetch property listings when tab changes, but prioritize localStorage
  useEffect(() => {
    // Force update state with localStorage data first
    const localStorageData = getLocalStorageProperties();
    console.log("Local storage data loaded:", localStorageData.length, "properties");
    
    // Only fetch from API if local storage is empty or we need to sync
    if (localStorageData.length === 0) {
      console.log("No local storage data, fetching from API...");
      if (activeTab === 'All') {
      ...
      console.log("Using localStorage data instead of API fetch");
    }
  }, [activeTab, dispatch]);
