            ...
            <div className="dropdown-menu">
              ...
              <Link 
                ...
                onClick={() => {
                  ...
                  // Fix for property address showing as [object Object] - enhanced
                  let propertyAddress = "";
                  if (property.property_address) {
                    if (typeof property.property_address === 'string') {
                      propertyAddress = property.property_address;
                    } else if (typeof property.property_address === 'object' && property.property_address !== null) {
                      // For object addresses, create a meaningful string representation
                      const address = property.property_address;
                      if (address.street || address.city || address.region) {
                        propertyAddress = `${address.street || ''}, ${address.city || ''}, ${address.region || ''}`.replace(/,\s*$/, '').replace(/,,/g, ',');
                      } else {
                        // Fallback for other object structures
                        const entries = Object.entries(address).filter(([_, v]) => v).map(([k, v]) => `${v}`);
                        propertyAddress = entries.join(", ");
                      }
                    } else {
                      propertyAddress = "Address information unavailable";
                    }
                  } else if (property.address) {
                    if (typeof property.address === 'string') {
                      propertyAddress = property.address;
                    } else if (typeof property.address === 'object' && property.address !== null) {
                      // For object addresses, create a meaningful string representation
