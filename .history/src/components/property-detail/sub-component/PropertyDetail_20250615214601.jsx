...
const PropertyDetail = ({ PropertyDetails, similarProperties }) => {
    ...
    // Fetch similar properties when PropertyDetails loads
    useEffect(() => {
        if (PropertyDetails && PropertyDetails._id) {
            // Dispatch action to get similar properties
            dispatch(getSimilarProperties(PropertyDetails));
        }
    ...
    useEffect(() => {
        if (similarPropertiesFromRedux && similarPropertiesFromRedux.length > 0) {
            ...
            // Fall back to any props passed in if available
            setNearbyProperties(similarProperties);
        }
    }, [similarPropertiesFromRedux, similarProperties]);
                        ...
                        {(!HomeList || HomeList.length === 0) && (
                            <div className="no-properties-message" style={{ 
                                ...
                                <p style={{ 
                                    ...
                                    color: '#666',
                                    marginBottom: '15px'
                                }}>No nearby properties found at this time.</p>
                                <Link to="/property-list" className="view-all-link" style={{
