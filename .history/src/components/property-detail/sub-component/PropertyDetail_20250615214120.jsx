...
const PropertyDetail = ({ PropertyDetails, similarProperties }) => {
    ...
    // Fetch nearby properties from the Redux store
    const dispatch = useDispatch();
    const homeData = useSelector((state) => state.home?.HomeData?.data);
    ...
    // Update nearby properties when homeData changes
    useEffect(() => {
        if (homeData && homeData.properties && homeData.properties.length > 0) {
            // Filter out the current property if it's in the list
            const filteredProperties = PropertyDetails?._id 
                ? homeData.properties.filter(prop => prop._id !== PropertyDetails._id)
                : homeData.properties;
            
            // Use up to 6 properties
            setNearbyProperties(filteredProperties.slice(0, 6));
        }
    }, [homeData, PropertyDetails]);
