...
const PropertyListForm = () => {
    ...
    const handleFileChange = async (event, index) => {
        ...
        if (file) {
            ...
            const filename = `local-image-${timestamp}-${index}-${file.name}`;
    ...
    const ImagesUpload = async (file, index, retryCount = 0) => {
        ...
        try {
            ...
            // Check legacy format - files array
                ...
                // Convert files array to expected image object format
                const formattedImages = files.map(file => {
                    return {
                        url: typeof file === 'string' ? file : file.url || file.path || file,
                        caption: typeof file === 'string' ? file.split('/').pop() : file.name || file.caption || file.url?.split('/').pop() || 'Uploaded image',
                        _id: file._id || `generated-${Date.now()}-${Math.random().toString(36).substring(7)}`
                    };
                });
            ...
            // Single file object
                ...
                const formattedImage = {
                    url: typeof files === 'string' ? files : files.url || files.path || files,
                    caption: typeof files === 'string' ? files.split('/').pop() : files.name || files.caption || files.url?.split('/').pop() || 'Uploaded image',
                    _id: files._id || `generated-${Date.now()}-${Math.random().toString(36).substring(7)}`
                };
