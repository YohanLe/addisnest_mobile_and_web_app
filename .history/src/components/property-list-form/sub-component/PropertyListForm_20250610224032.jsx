...
const PropertyListForm = () => {
    ...
    const ImagesUpload = async (file, index) => {
        console.log("ImagesUpload function called for index:", index);
        try {
            setLoading(true);
            ...
            console.log("Uploading image to media/upload...");
            ...
            if (response.images && Array.isArray(response.images)) {
                setMediaPaths(prev => {
                    const newMediaPaths = [...prev, ...response.images];
                    console.log("Updated MediaPaths with response.images:", newMediaPaths);
                ...
                const formattedImages = files.map(file => {
                    console.log("Processing file in map:", file);
                ...
                setMediaPaths(prev => {
                    console.log("Previous MediaPaths state:", prev);
                    const newMediaPaths = [...prev, ...formattedImages];
                    console.log("Updated MediaPaths with response.files:", newMediaPaths);
                    return newMediaPaths;
                });
            }
            ...
            console.error('Upload failed in ImagesUpload function:', err);
    ...
    let data = {
        ...
        address: {
            street: inps?.property_address || (testMode ? '123 Test Street, Addis Ababa' : ''),
