...
const PropertyListForm = () => {
    ...
    const handleFileChange = async (event, index) => {
        ...
        if (file) {
            ...
            // Create placeholder in expected format for immediate validation pass
            const timestamp = Date.now();
            const randomId = Math.floor(Math.random() * 1000000000);
            const filename = `${timestamp}-${randomId}-${file.name}`;
            const placeholderImageObj = {
                url: `/uploads/${filename}`,
                caption: filename,
                _id: `temp-${timestamp}-${index}`
            };
            
            // Add object to MediaPaths to ensure validation passes
            setMediaPaths(prev => {
                console.log("Adding placeholder image object to MediaPaths:", placeholderImageObj);
                return [...prev, placeholderImageObj];
            });
            ...
            // Still try to upload to server in background
    ...
    const ImagesUpload = async (file, index, retryCount = 0) => {
        ...
        try {
            ...
            console.log("Image upload API response (full):", response);
            toast.info("Image upload API response: " + JSON.stringify(response), { autoClose: 10000, style: { whiteSpace: 'pre-line', maxWidth: 600 } });
            const { files, status, message } = response;
            setLoading(false);
            
            setUploadingStates(prev => ({ ...prev, [index]: false }));
            
            // Check if the response contains images array (expected format)
            if (response.images && Array.isArray(response.images)) {
                // This is the ideal format - direct from server with correct structure
                setMediaPaths((prevPaths) => {
                    // Filter out any temporary placeholder with the same index
                    const filteredPaths = prevPaths.filter(path => {
                        return typeof path === 'string' || 
                               !path._id || 
                               !path._id.startsWith('temp-');
                    });
                    
                    const updated = [...filteredPaths, ...response.images];
                    console.log("MediaPaths after image upload (using images array):", updated);
                    return updated;
                });
            ...
            // Check legacy format - files array
            else if (files && Array.isArray(files)) {
                // Convert files array to expected image object format with proper paths
                const formattedImages = files.map(file => {
                    const filePath = typeof file === 'string' ? file : file.url || file.path || file;
                    const fileCaption = filePath.split('/').pop(); // Extract filename from path
                    
                    // Generate MongoDB-style ID if not present
                    const timestamp = Math.floor(Date.now() / 1000).toString(16); // First part of ObjectId
                    const randomHex = Math.floor(Math.random() * 0xffffff).toString(16).padStart(6, '0'); // Random section
                    const counter = Math.floor(Math.random() * 0xffffff).toString(16).padStart(6, '0'); // Counter simulation
                    const mongoStyleId = file._id || `${timestamp}${randomHex}${counter}`;
                ...
                setMediaPaths((prevPaths) => {
                    // Filter out any temporary placeholders
                    const filteredPaths = prevPaths.filter(path => {
                        return typeof path === 'string' || 
                               !path._id || 
                               !path._id.startsWith('temp-');
                    ...
                    const updated = [...filteredPaths, ...formattedImages];
                    console.log("MediaPaths after image upload (formatted):", updated);
                    return updated;
                });
            ...
            // Single file object
                ...
                const filePath = typeof files === 'string' ? files : files.url || files.path || files;
                const fileCaption = filePath.split('/').pop(); // Extract filename from path
                ...
                const randomHex = Math.floor(Math.random() * 0xffffff).toString(16).padStart(6, '0'); // Random section
                const counter = Math.floor(Math.random() * 0xffffff).toString(16).padStart(6, '0'); // Counter simulation
                const mongoStyleId = files._id || `${timestamp}${randomHex}${counter}`;
                ...
                setMediaPaths((prevPaths) => {
                    ...
                    const updated = [...filteredPaths, formattedImage];
                    console.log("MediaPaths after image upload (single formatted):", updated);
                    return updated;
                });
                ...
                toast.warning("No valid image data in API response. Full response: " + JSON.stringify(response), { autoClose: 15000, style: { whiteSpace: 'pre-line', maxWidth: 600 } });
            }
            ...
            toast.error("Image upload error: " + JSON.stringify(error), { autoClose: 10000, style: { whiteSpace: 'pre-line', maxWidth: 600 } });
