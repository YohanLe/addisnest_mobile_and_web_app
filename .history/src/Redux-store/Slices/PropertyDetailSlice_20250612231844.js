  ...
  async (propertyId, { rejectWithValue, dispatch }) => {
    try {
      ...
      const transformedData = {
        ...
        price: apiData.price?.amount || 0,
        address: `${apiData.location?.address || ''}, ${apiData.location?.city || ''}, ${apiData.location?.state || ''}`,
        propertyFor: apiData.propertyFor || 'sale',
        property_type: apiData.propertyType || { value: 'house', label: 'House' },
        property_size: apiData.specifications?.area?.size || 0,
        bathroom_information: apiData.specifications?.bathrooms ? Array(apiData.specifications.bathrooms).fill({}) : [],
        createdAt: apiData.createdAt || new Date().toISOString(),
        views: apiData.views || 0,
        media: apiData.images?.map(img => img.url) || [],
        features: apiData.features ? Object.entries(apiData.features).flatMap(([type, items]) => 
          items.map(item => ({ type, name: item }))
        ) : [],
        user: {
          id: apiData.agent?._id,
          name: `${apiData.agent?.firstName || ''} ${apiData.agent?.lastName || ''}`,
          profile_img: apiData.agent?.avatar || 'https://randomuser.me/api/portraits/men/32.jpg'
        }
      };
