...
const transformApiDataToPropertyDetail = (apiData) => {
  ...
  // Process features to handle different formats
  ...
  async (propertyId, { rejectWithValue }) => {
    try {
      ...
      try {
        ...
        const response = await Api.getPublic(`properties/mongo-id/${propertyId}?t=${Date.now()}`);
        ...
        try {
          ...
          const altResponse = await Api.getPublic(`properties/${propertyId}?t=${Date.now()}`);
          
          if (altResponse && altResponse.data) {
            console.log('Alternative endpoint succeeded');
            return transformApiDataToPropertyDetail(altResponse.data.data || altResponse.data);
          }
        } catch (altError) {
        ...
        try {
          ...
          const dbResponse = await Api.getPublic(`properties/direct-db-query/${propertyId}?t=${Date.now()}`);
          
          if (dbResponse && dbResponse.data) {
            console.log('Direct database query succeeded');
            return transformApiDataToPropertyDetail(dbResponse.data.data || dbResponse.data);
          }
        } catch (dbError) {
        ...
        return rejectWithValue('Failed to fetch property details after multiple attempts');
      }
    } catch (error) {
