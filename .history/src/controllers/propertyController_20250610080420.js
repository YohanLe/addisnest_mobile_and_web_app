...
class PropertyController extends BaseController {
  ...
  createProperty = this.asyncHandler(async (req, res) => {
    try {
      ...
      // Check if promotion_package is provided
      const isPaidListing = req.body.promotion_package && req.body.promotion_package !== 'basic';
      ...
      // Remove promotion_package from property data if present
      const { promotion_package, ...propertyData } = req.body;
  ...
  submitPropertyPending = this.asyncHandler(async (req, res) => {
    try {
      ...
      // Check if promotion_package is provided and if it's basic
      const isBasicPackage = req.body.promotion_package === 'basic';
      
      // Set status based on package type
      if (isBasicPackage) {
        // Basic package properties are immediately active
        req.body.status = 'active';
        req.body.paymentStatus = 'none';
        console.log('Setting property status to ACTIVE for basic package');
      } else {
        // Premium packages require payment
        req.body.status = 'pending_payment';
        req.body.paymentStatus = 'pending';
        console.log('Setting property status to PENDING_PAYMENT for premium package');
      }
