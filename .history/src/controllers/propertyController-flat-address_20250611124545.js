...
class PropertyController extends BaseController {
  ...
  createProperty = this.asyncHandler(async (req, res) => {
    try {
      ...
      // Handle address fields - convert from flat structure to nested structure for MongoDB
      const propertyData = { ...req.body };

      // If we have flat address fields, convert them to the nested address object
      if (propertyData.street || propertyData.city || propertyData.regional_state || propertyData.country) {
        propertyData.address = {
          street: propertyData.street || '',
          city: propertyData.city || '',
          state: propertyData.regional_state || '',
        ...
        delete propertyData.regional_state;
        
        // Keep country at top level for backward compatibility
        // Don't delete propertyData.country
      }
  ...
  getAllProperties = this.asyncHandler(async (req, res) => {
    ...
    // Transform the results to include flat address fields
    const transformedProperties = properties.map(property => {
      const propertyObj = property.toObject();
      
      // Add flat address fields if address object exists
      if (propertyObj.address) {
        propertyObj.street = propertyObj.address.street || '';
        propertyObj.city = propertyObj.address.city || '';
        propertyObj.regional_state = propertyObj.address.state || '';
        // Don't override country if it already exists at the top level
        if (!propertyObj.country) {
          propertyObj.country = propertyObj.address.country || 'Ethiopia';
        }
      }
  ...
  getPropertyById = this.asyncHandler(async (req, res) => {
    ...
    try {
      ...
      // Transform to include flat address fields
      const propertyObj = property.toObject();
      ...
      this.sendResponse(res, propertyObj);
    } catch (error) {
  ...
  updateProperty = this.asyncHandler(async (req, res) => {
    ...
    // If we have flat address fields, convert them to the nested address object
    if (updateData.street || updateData.city || updateData.regional_state || updateData.country) {
      // Get current address object to ensure we don't lose data
      ...
      updateData.address = {
        ...
        state: updateData.regional_state || currentAddress.state || '',
        country: updateData.country || currentAddress.country || 'Ethiopia'
      };
      
      // Remove flat fields to prevent duplication
      delete updateData.street;
      delete updateData.city;
      delete updateData.regional_state;
      
      // Keep country at top level for backward compatibility
      // Don't delete updateData.country
    }
    ...
    // Transform result to include flat address fields
    const propertyObj = property.toObject();
  ...
  submitPropertyPending = this.asyncHandler(async (req, res) => {
    try {
      ...
      this.sendResponse(res, propertyObj, 201);
    } catch (err) {
  ...
  getPropertiesByUser = this.asyncHandler(async (req, res) => {
    ...
    // Transform results to include flat address fields
    const transformedProperties = properties.map(property => {
      const propertyObj = property.toObject();
  ...
  getFeaturedProperties = this.asyncHandler(async (req, res) => {
    ...
    this.sendResponse(res, transformedProperties);
  });
  ...
  searchProperties = this.asyncHandler(async (req, res) => {
    ...
    // Search term - updated to support flat address fields
    if (q) {
      query.$or = [
        ...
        // Also support flat fields for newer records
        { city: { $regex: q, $options: 'i' } },
        { regional_state: { $regex: q, $options: 'i' } },
