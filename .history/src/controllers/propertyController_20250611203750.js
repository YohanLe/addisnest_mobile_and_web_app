...
class PropertyController extends BaseController {
  ...
  ensureAddressFields(data) {
    ...
    } else if (data.regional_state) {
        data.state = data.regional_state
    }
    else {
  ...
  sanitizePropertyData(data) {
    ...
    // Ensure features is an object and handle amenities
    if (data.amenities && Array.isArray(data.amenities)) {
      data.features = {};
      data.amenities.forEach(amenity => {
        data.features[amenity] = true;
      });
  ...
  createProperty = this.asyncHandler(async (req, res) => {
    try {
      ...
      console.log('Data before create:', req.body);
      // Create the property
