...
import EmailVerificationPopup from "../../helper/EmailVerificationPopup";
import CustomerRegisterPopup from "../../helper/CustomerRegisterPopup";
import { setToken } from "../../utils/tokenHandler";

/**
 * Login component that can be used as both a standalone page and a popup
 * @param {Object} props - Component properties
 * @param {boolean} props.isPopup - Whether to render as popup or page
 * @param {Function} props.handlePopup - Function to close popup (only needed for popup mode)
 */
const Login = ({ isPopup = false, handlePopup = () => {} }) => {
    ...
    const loginFun = async () => {
        ...
        if (errorMessage.isValid == false) {
            ...
            try {
                ...
                // Correctly extract the token from the response data
                const token = responseData?.token;

                if (!token || typeof token !== 'string' || token.trim() === "") {
                    toast.error("Login failed: No valid token received from server.");
                    return;
                }
                ...
                if (isPopup && activeTab === "agent") {
                    window.location.replace(
                        `${window.location.origin.replace(":5185", ":3001")}/`
                    );
                } else if (isPopup) {
                    // Close the popup first
                    handlePopup();
                    // Navigate to property list form
