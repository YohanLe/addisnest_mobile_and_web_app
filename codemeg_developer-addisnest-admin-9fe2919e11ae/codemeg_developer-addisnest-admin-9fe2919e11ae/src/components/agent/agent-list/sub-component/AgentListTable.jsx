...
import { fetchAgentList } from "../../../../Redux-store/Slices/AgentListSlice";
import Api from "../../../../Apis/Api";

const AgentListTable = () => {
  const dispatch = useDispatch();
  const { agents, loading, error, totalPages, currentPage: reduxCurrentPage } = useSelector(
  ...
  // Fetch agents when currentPage or debouncedSearchTerm changes
  useEffect(() => {
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      await Api.postWithtoken(`users/users/${agentIdToDelete}`);
      fetchAgents(currentPage, debouncedSearchTerm);
        ...
        <div className="card-body">
          ...
          <div className="responsive-table">
            <table className="table table-row-dashed">
              <thead>
                <tr>
                  <th className="w-250px text-start">Agent Name</th>
                        ...
                        <div className="action-main">
                          <div className="action-inner">
                            <div className="action-buttons">
                              <Link to={`/agent-detail/${item.id}`} className="view-action">
                                <SvgActionViewIcon />
                              </Link>
                            </div>
                          </div>
